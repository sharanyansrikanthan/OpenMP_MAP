...
/* MAPPER: MAPPER data sent to OMP */
struct MAPdata
{
	double progress; /* Progress metric if valid_progress = 1 */
	int valid_progress; /* 1 if function call is used to give progress feedback by application */
	int numthreads; /* Parallelism allocated by MAPPER to the application */
	int valid_threads; /* True if parallelism value is valid */
	int mode; /* Mode of parallelism control. Not actively used only for experimental evaluation */
} *MAPptr;
...
/* MAPPER: Main routine that checks MAPPER output and alters parallelism */
void *map_monitor()
{
	...
	do
	{
		/* MAPPER: Check if the MAPptr data structure has been allocated by the control
		 * threads. Normally it is allocated. */
		if (MAPptr) {
			/* Mode is 0 for regular operation and set by MAPPER */
			...
			if (MAPptr->valid_threads && MAPptr->numthreads) {
				*map_num_threads = MAPptr->numthreads;
				/* MAPPER: Allocated during initialization. Used for making changes 
				 * to control parallelism of application */
			}
		...
		/* MAPPER: Check if progress is available and monitor if it is */
...
/* MAPPER: Set the number of threads to be used for parallel computation */
void
map_set_num_threads (int n)
{
  struct gomp_task_icv *icv = gomp_icv (true);
	/* MAPPER: Set the global control variable */
  if (icv != NULL)
        icv->nthreads_var = (n > 0 ? n : 1);
  if (map_num_threads != NULL && icv != NULL)
  {
        *map_num_threads = icv->nthreads_var;
				/* MAPPER: map_num_threads will be used by the loops
...
/* MAPPER: Create data structures and launch the 
 * control thread dedicated to interacting with MAPPER */
int map_create_control ()
{
  ...
  printf("In main: creating thread \n");
	/* MAPPER: Open mmap needed to communicate between MAPPER and OpenMP */
...
/* MAPPER: Function called to initilize progress reporting */
double*
omp_team_progress()
{
  scale_add = 1;
...
initialize_env (void)
{
  ...
  pthread_attr_setdetachstate (&gomp_thread_attr, PTHREAD_CREATE_DETACHED);
	/* MAPPER: Initialize MAPPER related data structures */
	map_create_control();
