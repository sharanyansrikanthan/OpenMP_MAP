...
GOMP_parallel_start (void (*fn) (void *), void *data, unsigned num_threads)
{
  num_threads = gomp_resolve_num_threads (num_threads, 0);
	/* MAPPER: Control the parallelism of the OMP_Parallel region */
...
{
	/* MAPPER: Controll parallelism of the region */
	if (MAP_PRINT) printf("\n Team start %lu %d \n", (unsigned long)data, num_threads);
  map_omp_specify = num_threads;
  num_threads = gomp_resolve_num_threads (num_threads, 0);
  // num_threads = gomp_resolve_num_threads (num_threads, 0);
...
/* MAPPER: Return 0 if the thread ID is less than MAPPER chosen number of 
 * active threads. Return 1 otherwise. 
 * MAPPER chosen number of threads for this loop has to be changed latest by
 * the time the first thread of the loop calls this function or get_num_threads(). 
 * So the first thread locks in the degree of parallelism to be used for this loop
 * so that other threads read the same value. This value of parallelism is unlocked
 * by the barrier that releases all the threads to indicate the end of the loop
 */
...
omp_get_flag_shar (void)
{
  ...
  // printf("\n In get_flag_shar %d %d %u \n", tid, map_comp_flag, map_comp_threads);
	/* MAPPER: Flag to check if any other thread of this pool has set the number of threads 
	 * in the loop. If no other thread has set it, acquire the lock and set it. */
  if (map_comp_flag == 0)
  {
...
/* Returns the number of threads used for this particular loop/parallel region. 
 * This is used internally by the thread pool only. It is used to divide the work
 * statically between the number of threads involved in the computation.
 * The first thread that calls this function or omp_get_flag_shar() sets the number of 
 * threads to be used based on the MAPPER input read by the control thread launched in env.c
 */
