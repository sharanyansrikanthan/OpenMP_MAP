			       ...
			       gimple *inner_stmt)
{
  ...
  printf("In no chunk compile\n");
	/* MAPPER: GET_NUM_THREADS_SHAR is the hook to read the number of threads
	 * that is to be used for the parallel region */
  ...
  threadid = force_gimple_operand_gsi (&gsi, threadid, true, NULL_TREE,
				       true, GSI_SAME_STMT);
	/* MAPPER: OMP_GET_FLAG_SHAR will be called by every thread just like 
	 * GET_NUM_THREADS_SHAR but is responsible for disabling any thread IDs 
	 * in the thread pool that exceeds GET_NUM_THREAD_SHAR. 
	 * Only threads 1 - n will divide up the total work even if the
	 * total active thread pool is greater than n */
  ...
  e0 = force_gimple_operand_gsi (&gsi, t, true, NULL_TREE, true, GSI_SAME_STMT);
	/* MAPPER: fshar is used to send the excess threads straight to the barrier 
	 * that comes at the end of the current parallel computation. It tricks the 
	 * thread by ensuring that loop termination condition has occurred. */
